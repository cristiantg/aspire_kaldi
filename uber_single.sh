...
if [ -f "$audio_file" ]; then
    ...
    --config=${1}/exp/tdnn_7b_chain_online/conf/online.conf \
    --max-active=7000 \
    --beam=15.0 \
    --lattice-beam=6.0 \
    --acoustic-scale=1.0 \
    --word-symbol-table=${1}/exp/tdnn_7b_chain_online/graph_pp/words.txt \
    ${1}/exp/tdnn_7b_chain_online/final.mdl \
    ${1}/exp/tdnn_7b_chain_online/graph_pp/HCLG.fst \
    'ark:echo '$spk_id' '$utt'|' \
    'scp:echo '$utt' '$audio_file'|' \
    ark:- | lattice-to-ctm-conf ark:- $output_folder/$m_bestsym
                
    ${1}/utils/int2sym.pl -f 5 ${1}/exp/tdnn_7b_chain_online/graph_pp/words.txt $output_folder/$m_bestsym > $output_folder/$output_file_name
fi
